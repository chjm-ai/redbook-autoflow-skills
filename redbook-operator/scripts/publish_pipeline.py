...
def _write_result_file(result_file: str | None, payload: dict):
    """Persist publish status when the caller provides a result file."""
    if not result_file:
        return
    ...
    with open(abs_path, "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print(f"[pipeline] Result file written: {abs_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON file to persist publish status. Recommended path: "
            "../redbook-auto-flow/workspace/{run_id}/candidates/{candidate_id}/publish/publish_result.json"
        ),
    )
    parser.add_argument(
        "--skip-file-check",
