...
本目录可作为独立 skill 使用。安装原则只有一条：把包含 `SKILL.md` 的整个目录放到宿主工具的 skills 目录下。

例如：

- Claude Code 可放到 `~/.claude/skills/redbook-operator/`
- OpenClaw 可放到它自己的 skills 目录下
- 其他支持目录化 skill 的工具也按相同方式处理

如果你已经在使用支持安装 skill 的 AI 助手，也可以直接把这个目录交给它，让它安装到对应的 skills 目录。
...
## Stars
[![Stargazers over time](https://starchart.cc/white0dew/Redbook-Publish.svg?variant=adaptive)](https://starchart.cc/white0dew/Redbook-Publish)
